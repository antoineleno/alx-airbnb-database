...
SELECT p.name AS Property_Name, p.description,p.location, p.pricepernight, r.rating, r.comment FROM properties as p LEFT JOIN reviews AS r ON p.property_id=r.property_id ORDER BY p.created_at;
